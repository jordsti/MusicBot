...
class MusicBot(discord.Client):
    ...
    async def cmd_playalbum(self, player, channel, author, permissions, leftover_args, album):
        """
        Usage:
        ...
        if leftover_args:
            album = ' '.join([album, *leftover_args])

        db_entries = self.musicdb.search_album(album)
        if len(db_entries) > 0:
            for e in db_entries:
                # we got a local match
                print("Local match : {0}".format(e.file_path))
                await player.playlist.add_local_entry(e)
                procmsg = await self.safe_send_message(channel, "Track added : {0} - {1}".format(e.tag.track, e.tag.title))
        else:
